...
func New(cfg *Config) (*GRPCServer, error) {
	...
	if cfg.MigrationPath != "" {
		if err := database.RunMigrationsWithPrefix(l, cfg.MigrationPath, cfg.DBCluster); err != nil {
			return nil, fmt.Errorf("failed to run migrations for %s: %w", cfg.DBCluster, err)
		}
	}
...
func initRedisServer(ctx context.Context, logger logger.LoggerInterface, cfg *Config) (redis.UniversalClient, error) {
	...
	clusterMode := viper.GetBool("REDIS_CLUSTER_MODE")
	if clusterMode {
		db = 0 // Redis Cluster does not support non-zero DB selection
	}
