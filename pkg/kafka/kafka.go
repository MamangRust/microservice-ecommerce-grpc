...
func (k *Kafka) SendMessage(topic string, key string, value []byte) error {
	...
	k.logger.Debug("Message stored", zap.String("topic", topic), zap.Int32("partition", partition), zap.Int64("offset", offset))
	return nil
}
		...
		for {
			...
			if err != nil {
				...
				if retries >= maxRetries {
					k.logger.Fatal("Max retries reached for consumer group. Exiting.")
				}
