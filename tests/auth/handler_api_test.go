...
func (s *AuthHandlerApiTestSuite) SetupSuite() {
	...
	testRes, err := roledb.New(pool).GetRoles(context.Background(), roledb.GetRolesParams{
		Column1: "",
		Limit:   10,
		Offset:  0,
	})
	...
	roleByID, err := roledb.New(pool).GetRole(context.Background(), createdRoleRes.Data.Id)
	if err != nil {
