...
func NewServer(cfg *server.Config) (*server.GRPCServer, error) {
	...
	// Phase 6 — transactional outbox: email events are enqueued durably (the
	// register/verify flows enqueue after the remote user-service write) and the
	// relay publishes them with retry/dead-letter semantics.
