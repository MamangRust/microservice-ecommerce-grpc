...
func setupCategoryStatsHandler(deps *DepsCategory, mapper apimapper.CategoryStatsResponseMapper, cache category_cache.CategoryMencache) func() {
	return func() {
		NewCategoryStatsHandleApi(&categoryStatsHandleDeps{
			statsClient:           pb.NewCategoryStatsServiceClient(deps.StatsReader),
			statsByIdClient:       pb.NewCategoryStatsByIdServiceClient(deps.StatsReader),
			statsByMerchantClient: pb.NewCategoryStatsByMerchantServiceClient(deps.StatsReader),
			router:                deps.E,
			logger:                deps.Logger,
			mapper:                mapper,
			cache:                 cache,
