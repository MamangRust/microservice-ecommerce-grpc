...
import (
	...
	"github.com/MamangRust/microservice-ecommerce-grpc-apigateway/handler"
	"github.com/MamangRust/microservice-ecommerce-grpc-apigateway/middlewares"
	// Blank import: registers the generated Swagger spec (docs.init → swag.Register)
	// so echo-swagger's /swagger/doc.json can resolve swag.ReadDoc("swagger").
	_ "github.com/MamangRust/microservice-ecommerce-grpc-apigateway/docs"
...
func createConnection(address, serviceName string, logger logger.LoggerInterface) (*grpc.ClientConn, error) {
	...
	conn, err := grpc.NewClient(
		...
		// F6: dependency guard (per-call deadline + circuit breaker + bulkhead)
		// per downstream service, then propagate the current trace context to
		// downstream gRPC services so a single request can be traced
		// end-to-end from the gateway to the domain.
		grpc.WithChainUnaryInterceptor(
			pkgresilience.NewDependencyGuardInterceptor(logger).UnaryInterceptor(),
